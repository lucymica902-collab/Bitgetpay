...
app.listen(PORT, () => { console.log(`Server running on port ${PORT}`); });req.session.user = users.find(u => u.id === req.session.user.id) || req.session.user;
...
app.post('/admin/verify/:id', (req, res) => {
    ...
    if (tx && tx.status === 'Pending') {
        tx.status = 'Approved & Verified';const express = require('express');
